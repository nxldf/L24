...
$.extend(Datepicker.prototype, {
	...
	_generateHTML: function(inst) {
		...
		var controls = (!inst.inline ? '<button type="button" class="ui-datepicker-close btn" onclick="DP_jQuery_' + dpuuid +
			'.datepicker._hideDatepicker();">' + this._get(inst, 'closeText') + '</button>' : '');
		var buttonPanel = (showButtonPanel) ? '<div class="ui-datepicker-buttonpane ui-widget-content">' + (isRTL ? controls : '') +
			(this._isInRange(inst, gotoDate) ? '<button type="button" class="ui-datepicker-current btn" onclick="DP_jQuery_' + dpuuid +
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						tbody += '<td class="' +
							...
							(unselectable ? '<span class="ui-state-default">' + printDate.getDate() + '</span>' : '<a style="font-family:' + this._get(inst, 'font') + '" class="ui-state-default' +
